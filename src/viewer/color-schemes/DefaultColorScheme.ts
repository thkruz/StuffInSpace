...
class DefaultColorScheme extends ColorScheme {
  constructor () {
    super ('Default color scheme', (satellite: Satellite) => {
      let color = [1.0, 1.0, 0.0, 1.0];
      let pickable = false;

      if (satellite) {
        pickable = true;
        if (satellite.type === SpaceObjectType.PAYLOAD) {
          color = [1.0, 0.2, 0.0, 1.0];
        } else if (satellite.type === SpaceObjectType.ROCKET_BODY) {
          color = [0.2, 0.5, 1.0, 0.85];
          //  return [0.6, 0.6, 0.6];
        } else if (satellite.type === SpaceObjectType.DEBRIS) {
